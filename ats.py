...
st.set_page_config(
    page_title="ATS Resume Reviewer",
    page_icon="📄",
    layout="wide"
)
...
st.caption(
    ...
    "by comparing it against a Job Description."
)
...
with st.expander("ℹ️ How to Use This Tool (Click to Expand)", expanded=True):
    st.markdown(
        ...
        - 📝 A fully **ATS-Friendly Improved Resume**
          (ready for download in DOCX/PDF)

        ---

        💡 **Tip:** Use bullet points, action verbs, and clear headings
        in your resume to maximize ATS readability.
        """
    )
...
Your task is to analyze the Resume against the Job Description and provide
feedback on how well the resume matches the job requirements.

Identify the weaknesses and strengths of the resume against the job description.

Also provide alternative words that can be used in the resume to match the
Job Description.

Provide a response that is comprehensive and easy to understand for a job applicant.

Also provide a score out of 100 on how well the resume matches the job description,
and how to improve the score to be competitive.

Highlight the key skills and experiences that are relevant to the job description.

Provide a well-structured and formatted resume suggestion that is ATS-friendly
and matches the Job Description.

Use bullet points and proper headings.

Keep each section to 200-350 words max.

IMPORTANT:
You must provide the response in the following JSON format without any extra prose:
...
def get_llm_response(prompt: str) -> str:
    """Call Groq's LLM API and return raw text."""
...
_heading_pattern = re.compile(
    r'^\s*(#{1,6}\s+|[A-Z][A-Za-z &/+-]{2,40}:|[A-Z ]{3,40})\s*$'
)
...
st.caption(
    ...
    "and optimize it for ATS success."
)
...
if submitted:
    ...
    if not API_KEY:
        ...
        st.error(
            "No API key found. Please set GROQ_API_KEY in .env"
        )
        ...
        if not resume_text.strip():
            ...
            if not parsed:
                ...
                st.error(
                    "❌ Could not parse a valid JSON response."
                )
                ...
                with tabs[1]:
                    ...
                    st.subheader(
                        "📝 Improved Resume (ATS-Friendly)"
                    )

                    improved_resume = parsed.get(
                        "Improved Resume",
                        "N/A"
                    )
